...
class ForwardTacotronIE:
    def __init__(
        self, model_duration, model_forward, core, device="CPU", verbose=False
    ):
        ...
        self.duration_predictor_request = self.create_infer_request(
            self.duration_predictor_model, model_duration
        )

        self.forward_model = self.load_network(model_forward)
        self.forward_request = self.create_infer_request(
        ...
        self.is_multi_speaker = check_input_name(
            self.duration_predictor_model, "speaker_embedding"
        )
    ...
    def gather(a, dim, index):
        expanded_index = [
            ...
            if dim == i
                ...
                [-1 if i == j else 1 for j in range(a.ndim)]
            )
    ...
    def create_infer_request(self, model, path):
        ...
        log.info(
            "The ForwardTacotron model {} is loaded to {}".format(path, self.device)
        )
        return compiled_model.create_infer_request()

    def infer_duration(
        self, sequence, speaker_embedding=None, alpha=1.0, non_empty_symbols=None
    ):
        if self.is_attention:
            input_mask = self.sequence_mask(
                np.array([[non_empty_symbols]]), sequence.shape[1]
            )
        ...
        duration = (
            self.duration_predictor_request.get_tensor("duration").data[:] * alpha
        )

        duration = (duration + 0.5).astype("int").flatten()
        duration = np.expand_dims(duration, axis=0)
        preprocessed_embeddings = self.duration_predictor_request.get_tensor(
            "embeddings"
        ).data[:]
    ...
    def infer_mel(self, aligned_emb, non_empty_symbols, speaker_embedding=None):
        if self.is_attention:
            data_mask = self.sequence_mask(
                np.array([[non_empty_symbols]]), aligned_emb.shape[1]
            )
            pos_mask = self.pos_mask[
                :, :, : aligned_emb.shape[1], : aligned_emb.shape[1]
            ]
    ...
    def forward_duration_prediction_by_delimiters(self, text, speaker_embedding, alpha):
        ...
        if seq_len <= self.duration_len:
            ...
            while start_idx < seq_len:
                ...
                sub_sequence += [_symbol_to_id[" "]] * (
                    self.duration_len - len(sub_sequence)
                )
    ...
    def forward(self, text, alpha=1.0, speaker_id=19, speaker_emb=None):
        ...
        while start_idx < aligned_emb.shape[1] and end_idx < aligned_emb.shape[1]:
            ...
            mel = self.infer_mel(
                sub_aligned_emb, end_idx - start_idx, speaker_embedding
            )
