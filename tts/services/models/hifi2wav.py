...
class WaveRNNIE:
    def __init__(
        ...
        if upsampler_width > 0:
            ...
            self.upsample_model.reshape(
                {"mels": PartialShape([orig_shape[0], upsampler_width, orig_shape[2]])}
            )

        self.upsample_request = self.create_infer_requests(
            self.upsample_model, model_upsample
        )

        self.rnn_model = self.load_network(model_rnn)
        self.rnn_requests = self.create_infer_requests(
            self.rnn_model, model_rnn, batch_sizes=self.batch_sizes
        )
    ...
    def forward_upsample(self, mels):
        ...
        upsample_mels = self.upsample_request.get_tensor("upsample_mels").data[
    ...
    def forward_rnn(self, mels, upsampled_mels, aux):
        ...
        if b_size not in self.batch_sizes:
            raise Exception(
                "Incorrect batch size {0}. Correct should be 2 ** something".format(
                    b_size
                )
            )
        ...
        for i in range(seq_len):
            ...
            self.rnn_requests[active_network].infer(
                inputs={
                    "m_t": m_t,
                    "a1_t": a1_t,
                    "a2_t": a2_t,
...
class MelGANIE:
    def __init__(self, model_name, ie, device="CPU", default_width=800):
        """
        return class provided MelGAN inference.

        :param model_name: path to xml with MelGAN model of WaveRNN
        ...
        self.input_name = "mel"
        self.MAX_WAV_VALUE = (
            32768.0  # constant for normalizing audio output in training stage
        )

        self.model = self.load_network(model_name)
        self.model.reshape({self.input_name: PartialShape([1, 80, -1])})

        self.request = self.create_infer_request(self.model, model_name)
